...
top_level as (
    ...
    from hits,
    jsonb_to_record(data) as data(hit jsonb, time float, event text)
),

targets_raw as (
    select
        event.event_id,
        hit.target as target

    from top_level as event,
    jsonb_to_record(event.hit) as hit(target jsonb)
),

weapons_raw as (
    select
        event.event_id,
        hit.weapon as weapon

    from top_level as event,
    jsonb_to_record(event.hit) as hit(weapon jsonb)
),

initiator_raw as (
    select
        event.event_id,
        hit.initiator as initiator

    from top_level as event,
    jsonb_to_record(event.hit) as hit(initiator jsonb)
),
